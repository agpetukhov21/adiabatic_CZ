...
# from pulse_shaping.contrib.examples.cz_utils import cz_schedule
...
# rise_time_ns = 5.0  # Gaussian 10-90 rise time for the bias pulses, in ns.
...
def plot_nulled_states():
    ...
    plt.figure()
    plt.plot(cp_f10_range, np.abs(np.diff(sub_evals, axis=-1)))
    plt.xlabel(r'Coupler $f_{10}$ (GHz)')
    plt.ylabel(f'Energy splitting of {nulled_states[0]}, {nulled_states[1]}')
    ylim = plt.ylim()
...
plt.figure()
schedule.plot_pulses(params)
plt.title('Example pulse')
...
print(fitted_anglesf)
print(duration)
plot_angles({k: v for k, v in fitted_angles.items()
             if k in (FSimAngles.CZ, FSimAngles.SWAP, FSimAngles.LEAKAGE)},
            times)
plt.show()
